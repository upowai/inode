...
logging.basicConfig(
    level=logging.INFO if "--nologs" not in sys.argv else logging.WARNING
)
...
def get_json(obj):
    return json.loads(
        json.dumps(
            obj, default=lambda o: getattr(o, "as_dict", getattr(o, "__dict__", str(o)))
        )
    )
...
def normalize_block(block) -> dict:
    ...
    block["timestamp"] = int(
        block["timestamp"].replace(tzinfo=timezone.utc).timestamp()
    )
    return block
...
def get_transaction_type_from_message(message: bytes) -> TransactionType:
    ...
    try:
        ...
        transaction_type = transaction_type_mapping.get(
            str(decoded_message), TransactionType.REGULAR
        )
...
def point_to_bytes(
    point: Point, address_format: AddressFormat = AddressFormat.FULL_HEX
) -> bytes:
    if address_format is AddressFormat.FULL_HEX:
        return point.x.to_bytes(32, byteorder=ENDIAN) + point.y.to_bytes(
            32, byteorder=ENDIAN
        )
...
def bytes_to_point(point_bytes: bytes) -> Point:
    if len(point_bytes) == 64:
        x, y = int.from_bytes(point_bytes[:32], ENDIAN), int.from_bytes(
            point_bytes[32:], ENDIAN
        )
...
def point_to_string(
    point: Point, address_format: AddressFormat = AddressFormat.COMPRESSED
) -> str:
    if address_format is AddressFormat.FULL_HEX:
        ...
        address = base58.b58encode(
            (42 if y % 2 == 0 else 43).to_bytes(1, ENDIAN) + x.to_bytes(32, ENDIAN)
        )
